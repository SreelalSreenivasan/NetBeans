...
<%
    ...
    String show = (String) request.getParameter("stime_ragam");
    {
        Class.forName("com.mysql.jdbc.Driver");
        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/omtrs", "root", "root");
        Statement st = con.createStatement();
        ResultSet rs = st.executeQuery("SELECT * FROM stime_ragam");
        ...
        switch(show)
        {
            case "Noon Show":{
                if(status.stime_noon>0)
                    response.sendRedirect("process_ragam_noon.jsp");
                else
            ...
            case "Matinee":{
                if(status.stime_noon>0)
                    response.sendRedirect("process_ragam_matinee.jsp");
                else
                    response.sendRedirect("index.html");
                break;
            }
            case "First Show":{
                if(status.stime_noon>0)
                    response.sendRedirect("process_ragam_first.jsp");
                else
                    response.sendRedirect("index.html");
                break;
            }
            case "Second Show":{
                if(status.stime_noon>0)
                    response.sendRedirect("process_ragam_second.jsp");
                else
                    response.sendRedirect("index.html");
                break;
            }
